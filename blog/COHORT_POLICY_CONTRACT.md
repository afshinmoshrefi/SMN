...
Material qualifications bind each actual comparison independently. `cohort.era_sensitive` cites only recent and earlier cycle summaries when those medians contrast. `cohort.annual_recency_5` and `cohort.annual_recency_10` appear only for contrasting disjoint annual comparisons and cite their respective `/recent/5` or `/recent/10` object. `cohort.genuine_contrast` preserves the same-span cycle/noncycle contrast independently of the main classification. Consistent comparisons do not become mandatory qualifications merely because another comparison differs. Multiple qualifications may share one natural passage and supporting explanation; every actual contradiction and its precise evidence reference remains required.
