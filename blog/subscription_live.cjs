...
const defects=require('./layout_defects.cjs');const landing=[];
const assert=(ok,msg)=>{if(!ok)throw Error(msg)};
(async()=>{
 browser=await chromium.launch({channel:process.env.SMN_BROWSER_CHANNEL??'chrome',headless:true});const page=await browser.newPage({viewport:{width:1440,height:1050}});
 ...
 landing.push(...(await page.evaluate(defects)).map(d=>'desktop '+d));
 await page.screenshot({path:path.join(R,'live-edition-desktop.png'),fullPage:true});
 await page.setViewportSize({width:390,height:844});landing.push(...(await page.evaluate(defects)).map(d=>'mobile '+d));
 await page.screenshot({path:path.join(R,'live-edition-mobile.png'),fullPage:true});
 assert(!landing.length,'Landing visible defects '+JSON.stringify(landing));
 const shot=n=>crypto.createHash('sha256').update(fs.readFileSync(path.join(R,n))).digest('hex');
 fs.writeFileSync(path.join(R,'live-landing-visual-checks.json'),JSON.stringify({passed:true,method:'automated_live_checks',screenshots:{'live-edition-desktop.png':shot('live-edition-desktop.png'),'live-edition-mobile.png':shot('live-edition-mobile.png')}},null,2));
 ...
 for(const e of entries){
  ...
  for(const [kind,width,height] of [['desktop',1440,1050],['mobile',390,844]]){
   ...
   const found=await page.evaluate(defects);assert(!found.length,e.symbol+' '+kind+' visible defects '+JSON.stringify(found));
