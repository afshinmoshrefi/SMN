...
For date-only events, a materially new development may propose an update on the same calendar date even though the timestamp is equal. Coverage must retain `source_body_hashes` (canonical URL to inspected body hash) and `covered_claim_texts` from the previously covered evidence. Discovery includes the current code-owned body hashes on each event. An equal-date update requires a changed body on a previously known source, an exact-span-grounded claim set whose fingerprint matches the new development ID, and supporting text that is not already contained in the previously covered claims. A renamed event or a different quote from an unchanged page is insufficient. Legacy coverage without these snapshots remains skipped for equal timestamps. Already covered developments and older events remain excluded. Neither the strict full-event-date rule nor the 48-hour cutoff was relaxed by this correction. These checks establish changed supporting evidence; semantic materiality still requires editorial review.
