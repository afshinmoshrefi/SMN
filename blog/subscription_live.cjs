...
(async()=>{
 ...
 const originals=await page.locator('.edition-original').evaluateAll(a=>a.map(x=>x.href));
 assert(entries.every(e=>originals.includes(e.production_original)),'Production comparison navigation');
 ...
 for(const e of entries){
  ...
  for(const [kind,width,height] of [['desktop',1440,1050],['mobile',390,844]]){
   ...
   assert(state.noindex==='noindex,nofollow'&&!state.body.includes('Editorial review has not passed'),e.symbol+' final Dev status');
