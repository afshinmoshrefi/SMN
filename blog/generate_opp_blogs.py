...
def login_appserver(keyprovider_token):
    ...
    print('login_appserver returned =',result)
...
#---------------------------------------------------------------------------------------------------------------
# 8/10/2023
# updated opp with more columns:  opp = opp[['date', 'symbol', 'daysOut', 'lOrS', 'sharpe_ratio','avg_profit','median_profit','cumulative_return','stddev']]
...
def create_html_table(df,dfs,domain_root,fi,years,post_title): # domain_root used for hrefs in <a> - dfs has the company names used for slug
    
    
     # creating the link for when the visual/detail switch is clicked
    opp_list_link = slugify(post_title+'_t') # _t is for having a new slug for opp_list blog with thumbnails   
   ...
   <div style='width:10rem'> <a href='{opp_list_link}'><img src='{config.domain_root}wp-content/uploads/2024/01/detail.png'></a></div>
    ...
    <details>
      <summary style='margin-bottom: 20px;'>
        <h2 class='report-h2-info'>
          Click an Opportunity the for Detailed Report<span class='info-circle'><i>i</i></span>
        </h2>
      </summary>
...
The top 10 Wave opportunities for {fi} are discovered by TradeWave alogrithm by searching for the highest probability financial instruments on <span style='color:red;font-weight:bold'>{df['Date'].iloc[0]}</span>.  Each opportunity lists the information needed for trading the financial instrument:
</p>
<h3 class='report-h3'>Date1</h3>
<p style="background-color: lightgray; color:black;padding:5px;">
Date1 is the start date of the Wave opportunity.  TradeWave provides top Wave opportuniteis every day listed by the start-date.  date1 and start-date are equivalent in TradeWave.
</p>
...
<p style="background-color: lightgray; color:black;padding:5px;">
Date2 or end date is the end date of the wave opportunity.  Date 2 is derived by adding Days to Date1.  
</p>
...
<h3 class='report-h3'>Detail Visual toggle</h3>
<p style="background-color: lightgray; color:black;padding:5px;">On the detailed report page, you'll find a convenient toggle switch at the top, allowing you to switch between two viewing modes: visual and detail. This toggle switch provides you with flexibility in how you explore the information presented.</p>
<p>By default, the toggle switch is set to the visual mode, which presents the top 10 trading opportunities with accompanying thumbnails. This mode gives you a quick and visually appealing overview of the opportunities, making it easy to identify ones that pique your interest.</p>
    ...
    if show_mobile_table:
        ...
        html_table = html_table + f"""
              ...
              <p style='font-size:1rem;'>
                  <a target='_blank' href='{config.domain_root}tradewave-analytics-101'>TradeWave Analytics 101</a>
...
def run_opp_blog_generation(id,opp_date,html_tables_only=False):
    ...
    html_table=create_html_table(df,dfs,config.domain_root,config.available_resources[str(id)],years,post_title) # dfs has the company names
    ...
    # make sure folders are created before creating or editng the file html_file_path
    folder_path = os.path.dirname(html_file_path)
    # Create all subfolders recursively if they don't exist
