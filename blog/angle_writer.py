...
The bounded loop from ANGLE_ENGINE_DESIGN.md §4, generation side:
    ...
    PLAN (LLM, JSON; one re-ask on invalid JSON; may veto once)
      ...
      -> on factual/material editorial issues: ONE edit, re-assemble, re-gate
...
def _prose_checks(prose: str, plan: Dict[str, Any], research: Any) -> Dict[str, Any]:
    """Check what the model wrote before assembly can hide invalid citations.

    Length excludes slot-generated tables, captions, source lists and method
    notes. The planned budget covers all author-written visible text.
    """
    text = re.sub(r"<(script|style)\b[^>]*>.*?</\1\s*>", "", prose, flags=re.I | re.S)
    text = re.sub(r"\{\{[^{}]+\}\}", " ", text)
    text = html_lib.unescape(re.sub(r"<[^>]*>", " ", text))
    words = len(re.findall(r"\S+", text))
    limit = int(int(plan.get("word_budget") or 0) * 1.1)
    issues = []
    if limit and words > limit:
        issues.append({"code": "LENGTH_OVER_BUDGET",
                       "detail": f"Author prose has {words} words; limit is {limit}. "
                       "Cut repeated statements and the least useful beat; retain the answer and material risks."})
    sources = research.get("sources", []) if isinstance(research, dict) else []
    known = {str(s.get("id")) for s in sources if isinstance(s, dict)}
    planned = {str(s) for s in plan.get("source_ids", [])}
    cited = set()
    for sup in re.findall(r"<sup\b[^>]*>(.*?)</sup\s*>", prose, flags=re.I | re.S):
        cited.update(re.findall(r"\b\d+\b", sup))
    invalid = cited - (known & planned)
    if invalid:
        issues.append({"code": "CITATION_UNPLANNED_OR_MISSING",
                       "detail": "These prose source IDs are absent from the plan or research: "
                       + ", ".join(sorted(invalid)) + ". Remove unsupported claims; do not invent citations."})
    return {"word_count": words, "word_limit": limit, "issues": issues}


def research_for_rendered_citations(research: Any, cited_ids: List[int]) -> Any:
    """Use the same citation identity as the assembled, cited-only source list."""
    if not isinstance(research, dict):
        return research
    from article_sources import normalize_research_source_ids
    aligned = copy.deepcopy(research)
    by_id = {str(s.get("id")): s for s in aligned.get("sources", []) if isinstance(s, dict)}
    aligned["sources"] = [by_id[str(s)] for s in cited_ids if str(s) in by_id]
    return normalize_research_source_ids(aligned)


def build_editorial_facts(card: Dict[str, Any], plan: Optional[Dict[str, Any]] = None,
                          cited_ids: Optional[List[int]] = None,
                          prose_metrics: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Shared fact identity for writing and any final title/HTML review."""
    plan = plan or {}
    cited_ids = cited_ids or []
    prose_metrics = prose_metrics or {"word_count": None, "word_limit": None}
    ...
    facts = {"symbol": card.get("symbol"),
             ...
             "editorial_plan": plan,
             "author_prose_words": prose_metrics["word_count"],
             "author_prose_word_limit": prose_metrics["word_limit"],
             "source_id_mapping": {str(old): index + 1 for index, old in enumerate(cited_ids)},
             ...
             "auxiliary_cells": [
                 {k: v for k, v in (a or {}).items()
                  if k in ("days", "years", "lookback_label", "role",
                           ...
                           "direction", "anchor_date", "evidence")}
...
def generate_angle_article(card: Dict[str, Any], *,
    ...
    status: 'ready' (all gates clean) | 'hold' (material issues survived the one
    ...
    def _editorial(html: str, cited_ids: List[int], prose_metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not run_editorial:
            return None
        from editorial_review import review_article
        facts = build_editorial_facts(card, plan, cited_ids, prose_metrics)
        return review_article(html, facts, research_for_rendered_citations(research, cited_ids), send=editorial_send)

    def _gate(assembled: Dict[str, Any], prose: str, tag: str) -> Tuple[List[Dict[str, str]], bool]:
        """Union of factual errors, material editorial issues and prose checks."""
        html = assembled["html"]
        # Chrome is deterministic and has its own length. Enforce the writer's
        # budget on writer text only, not the assembled document's scaffolding.
        integrity = validate_cell_article(html, card, word_budget=0)
        prose_metrics = _prose_checks(prose, plan, research)
        result: Dict[str, Any] = dict(integrity)
        result["prose"] = prose_metrics
        hold = False
        try:
            review = _editorial(html, assembled.get("cited_ids", []), prose_metrics)
        ...
        if review is not None:
            ...
            hard.extend(material_soft_issues(review))
            if review.get("decision") == "repair" and not hard:
                hard.append({"code": "EDITORIAL_REPAIR_UNSPECIFIED",
                             "detail": "Reviewer requested repair without an actionable issue. "
                             "Check the thesis, source support and redundant passages once."})
        result["revision_issues"] = hard
        result["ok"] = not hard and not hold
        return hard, hold

    assembled = _assemble(prose)
    artifacts["assembly_warnings"] = assembled["warnings"]
    html = assembled["html"]
    hard1, hold1 = _gate(assembled, prose, "gate1")
    ...
    # ---- ONE bounded revision on the union of factual/editorial issues ----
    ...
    hard2, hold2 = _gate(assembled2, prose2, "gate2")
    if hard2 or hold2:
        artifacts.update(status="hold", html=html)
        return artifacts
    artifacts.update(status="ready", html=html)
