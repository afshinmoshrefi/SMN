...
def get_opp_list_for_display(num_in_list,id,month,day,years,pyears,appserver_token):  # num_in_list = 10 would return top 10 opportunities
    ...
    for i,r in df.iterrows():
        ...
        # print('xxxxxxxxxxxxxxxx=',id,r['Date'],r['Symbol'],r['DaysOut'],years)
        if x['ChartData4'] == []: 
            print(f'for securities group id={id} chartData4 returned an empty list')
            exit()
...
def create_html_table(df,domain_root,resource_id,post_title,opp_date): # domain_root used for hrefs in <a> - dfs has the company names used for slug
    ...
    <br>
    <a target='_blank' href='{link_top10_by_market}'><b>Click to View Patterns by Market - {opp_date}</b></a>
...
#-------------------------------------------------------------------------------------------------------------------
def load_top10_based_on_AI(opp_date):
...
if __name__ == '__main__':
    ...
    if post_id == -1: # -1 is not found
        df = load_top10_based_on_AI(opp_date)
        # df.to_csv('df.csv')
