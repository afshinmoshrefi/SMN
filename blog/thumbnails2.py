...
# line 459 changed Trade to Pattern 5/3/2024
...
def create_thumbnail(tnd):
    ...
    td = {
        ...
        'rect_width_pct'   : tnd['banner_text_width'],
        'rect_x_pct'       : 0.5,   # location of the center of the banner x
        'rect_y_pct'       : tnd['banner_top_pct'], # location of the center of the banner y - top:0.075  center:0.5 bottom:0.925
    ...
    logo_x   = (image.size[0] * (1-0.8)/2 - logo_h_w)/2
    logo_x   = int(logo_x)
    ...
    #####################################################
    rect_y_pct = 0.525
    if 'row2_top_pct' in tnd: 
        rect_y_pct = tnd['row2_top_pct']
    td = {
        'rect_height_pct':tnd['info_rect_height_pct'],
        'rect_width_pct':tnd['info_rect_width_pct'],
        'rect_x_pct':1-(tnd['info_rect_width_pct']/2),
        'rect_y_pct':rect_y_pct, # top:0.075  center:0.5 bottom:0.925
    ...
    rect_y_pct = 0.7475
    if 'row3_top_pct' in tnd: 
        rect_y_pct = tnd['row3_top_pct']
    td = {
        'rect_height_pct':tnd['info_rect_height_pct'],
        'rect_width_pct':tnd['info_rect_width_pct'],
        'rect_x_pct':1-(tnd['info_rect_width_pct']/2),
    ...
    if 'row4_top_pct' in tnd: 
        row4content = tnd['row4_text']
        rect_y_pct = tnd['row4_top_pct']
        td = {
            'rect_height_pct':tnd['info_rect_height_pct'],
            'rect_width_pct':tnd['info_rect_width_pct'],
            'rect_x_pct':1-(tnd['info_rect_width_pct']/2),
            'rect_y_pct':rect_y_pct, # top:0.075  center:0.5 bottom:0.925
            'rect_color':tnd['row4_bg_color'] ,
            'font_color':tnd['row4_font_color'],
            'text_width_factor':tnd['text_width_factor'], # 0.8 means text width is 80% of the rect width
            'text_content':row4content,
            'font_ttf':tnd['font_ttf'],
            'rotate_angle':0
        }
        image = draw_text_rect(image,draw,td)
...
#-------------------------------------------------------------------------------
    if tnd['disclaimer_text1'] != '':  # treat it as 2 lines disclaimer vs 1 line
    ...
    else:  # this is 1 line disclaimer

        td = {
            'rect_height_pct':tnd['disclaimer_height_pct'],
            'rect_width_pct':1,
            'rect_x_pct':0.5, # 0.9 for right and 0.09 for left
            'rect_y_pct':0.945, # top:0.075  center:0.5 bottom:0.925
            'rect_color':(235,235,235) ,
            'font_color':(128, 128, 128, 128),
            'text_width_factor':0.95, # 0.8 means text width is 80% of the rect width
            'text_content':tnd['disclaimer_text'],
            'font_ttf':tnd['font_ttf'],
            'rotate_angle':0
        }
...
# this is the old style of bakcground person with a banner disclaimer and 3 banners on the right side
def create_socialmedia_thumbnail_type1(sm,resource_id,date,symbol,days,dir,avg_gain,years,title_pre,category = config.category_date_range_report,ttype='report'): # ttype could be report or video - changes background image
    ...
    banner_text = f'{title_pre}{dir.capitalize()} Wave Pattern in {company}'
    ...
    return filename,url
#--------------------------------------------------------------------------------------------------
# the new one goes here - thumbnail_variation is where we add variety of styles
def create_socialmedia_thumbnail_new(sm,thumbnail_variation,resource_id,date,symbol,days,dir,avg_gain,years,title_pre,category = config.category_date_range_report,ttype='report'): # ttype could be report or video - changes background image
    ...
    # banner_text = f'{title_pre}{dir.capitalize()} Wave Trade in {company}'
    banner_text = f'${symbol}'
    banner_font_color = ''
    if dir.lower() == 'long':
        banner_font_color = 'green'
    else:
        banner_font_color = 'red'

    row1_text   = f'{avg_gain} Avg Gain | {years} of {years}'
    row2_text   = f'Seasonal Edge | {xdate1} --> {xdate2} ({days} Days) '
    row3_text   = f'Sharpe          %Profitable          Cumulative'
    row4_text   = f' 1.31      100%       276%'
    # banner_text = 'News: Long Seasonal Trade in DexCom'
    # row1_text = 'Buy DXCM On May 29th'
    # row2_text = 'Sell DXCM By Aug 13th'
    # row3_text = 'AVG Gain: 29.5%'
    ...
    # default values
    info_rect_width_pct = 1
    text_width_factor = 0.5
    logo_factor = 0.7
    person_shift_left_pct = 0
    ...
    tnd={
        ...
        'text_width_factor'    : text_width_factor,  # means text will take 80% of the width of the rectangle
        'banner_text'          : banner_text,
        'disclaimer_height_pct': 0.1,
        'disclaimer_bg_color'  : (230,230,230),
        'disclaimer_text'      : f'Historical Analysis.  Not Investment Advice.',
        
        'disclaimer_text1'     : '',
        'disclaimer_text2'     : '',
        'banner_text_width'    : 0.6,
        'banner_font_color'    : banner_font_color,
        'banner_top_pct'       : 0.175,
        # 'banner_bg_color'      : generate_random_dark_color(),
        'banner_bg_color'      : 'transparent',
        'banner_font_color'    : banner_font_color,
        'row1_text'            : row1_text,
        'row1_bg_color'        : 'transparent',
        'row1_font_color'      : 'white',
        'row1_top_pct'         : 0.35,
        'row2_text'            : row2_text,
        'row2_bg_color'        : 'transparent',
        'row2_font_color'      : 'white',
        'row2_top_pct'         : 0.46,
        'row3_text'            : row3_text,
        'row3_bg_color'        : 'transparent',
        'row3_font_color'      : 'white',
        'row3_top_pct'         : 0.57,
        'row4_text'            : row4_text,
        'row4_bg_color'        : 'transparent',
        'row4_font_color'      : 'white',
        'row4_top_pct'         : 0.63,
        'ttype'                : ttype, # this is either report or video
        'social_media'         : sm # used to conditionally put the word video only on the facebook video thumbnails
    }
...
#--------------------------------------------------------------------------------------------------        
# this is a new create_socialmedia_thumbnail for better thumbnails - the old style is create_socialmedia_thumbnail_type1
def create_socialmedia_thumbnail(sm,resource_id,date,symbol,days,dir,avg_gain,years,title_pre,category = config.category_date_range_report,ttype='report'): # ttype could be report or video - changes background image
    thumbnail_variation = 2

    if thumbnail_variation == 1:
        return create_socialmedia_thumbnail_type1(sm,resource_id,date,symbol,days,dir,avg_gain,years,title_pre,category = config.category_date_range_report,ttype='report')

    return create_socialmedia_thumbnail_new(sm,thumbnail_variation,resource_id,date,symbol,days,dir,avg_gain,years,title_pre,category = config.category_date_range_report,ttype='report')
#--------------------------------------------------------------------------------------------------
