# this script finds the best with 100% profitability 
# start looking at 20/20 then 19/19 then 18/18 all the way to 10/10 from wilshire only and put it
# in a single file

# IMPORTANT SCREW UP NOTE
# here is a screw up - the dataframe here has 2 column for average profit 
# avg_profit - is the average profit overall includes the years with loss
# AVG Profit - is the average profit for the winning years, which we use in our stats display
# DON'T MIX THEM UP
...
def get_opp_list_for_display(num_in_list,id,month,day,years,pyears,appserver_token):  # num_in_list = 10 would return top 10 opportunities
    ...
    print('opp list loaded ',years,pyears)
    # print('opp_dict=',opp_dict)

    if 'OppList4' in opp_dict:
        v = opp_dict['OppList4']
        s = v.split(':')
        if s[0] == '-1':
            print(f'oppoturnity file missing: {s[1]}')
            return pd.DataFrame()
    ...
    top_opps=opp_dict['OppList'][:num_in_list] 
    ...
    for i,r in df.iterrows():
        ...
        # print(r['Date'],r['Symbol'],days_out,years)
    ...
    #     print(i,x['stats']['Cumulative Return'],x['stats']['Avg Profit'],x['stats']['Long Score'],x['stats']['Short Score'])
        ...
        last_trade_date.append(x['stats']['last_trade_date'])
    ...
    df['median_profit'] = df['median_profit'].astype(str) + '%'
    df['cumulative_return'] = df['cumulative_return'].astype(str) + '%'
...
# this is actually last 40 years - just a bad name for the function
def get_top_20year_info(opp_date):
    ...
    num      = 100 # we'll try for up to the top 100 - capped the number of opportunities per opportinity list

    top_20year_opp_df = {}

    id = 4 # this is wilshire

    # company names 
    symbols_csv = config.available_resources_path[str(id)]
    dfs = pd.read_csv(symbols_csv)[['symbols','name']]

    # for years in range(11,10,-1): # for debug onlyh # fix here after debug
    for years in range(40,9,-1):

        pyears = years  # we are looking at 100% of years profitable so years and pyears are equal
        
        
        if years >=15 :
            pyears = int(years*0.93) 
            if years == 15: pyears = 14
            # pyears = years - 1



        print('years and pyears =',years,pyears)
        ...
        if df.shape[0] == 0: # this combination of year and pyear doesn't exist
            continue
        ...
        #     symbol = r['Symbol']
        #     c=''
        #     df_symbol = dfs[dfs['symbols']==symbol]
        #     if df_symbol.shape[0] == 1: 
        #         c = df_symbol['name'].iloc[0]
        #     company.append(c)
        # df['company']=company

        dfs = dfs.rename(columns={'name':'company','symbols':'Symbol'})
        df = df.merge(dfs, on='Symbol', how='left')
        ...
        df['years'] = years # saving the number of years and pyears for each opportunity
        ...
        # print(financial_groups[id])        
        if df.shape[0] > 0:
            top_20year_opp_df[years]=df
    ...
    return top_20year_opp_df
        
#------------------------------------------------------------------------------------------------
# loads the data by generating it - 
# its actually last 40 years - this is just a bad name for the function
def load_top_20year(filename,opp_date):
    ...
    if file_exists:
        ...
        # check the dates inside this file 
        dft = pd.read_csv(filename)
        dates = list(set(list(dft['Date'])))
        print(dates)

        if opp_date in dates: # only generate if the opp_date is not in the filename csv - otherwise we load the csv instead of generating it
            action = 'load'
    ...
    if action == 'generate':
        ...
        top_20year_opp_df=get_top_20year_info(opp_date)

        dfc = pd.concat(top_20year_opp_df,ignore_index=True)



        dfc['Sharpe Ratio']=dfc['Sharpe Ratio'].astype(float)
        # dfc = dfc[dfc['Sharpe Ratio']>1.5]
...
if __name__ == '__main__':
    ...
    filename = config.top_20year_data[:-4]+'_'+opp_date+'.csv'
    ...
    dfc = dfc[dfc['Sharpe Ratio']> 1.5].reset_index(drop=True)
